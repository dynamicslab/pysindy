...
class SpectralDerivative(BaseDifferentiation):
    ...
    Works well for derivatives in periodic dimensions. Equivalent to a maximal-order finite difference, but runs in O(NlogN).
    ...
    def _differentiate(self, x, t):
        ...
        if x.dtype is complex:
            return np.fft.ifft(np.reshape(1j*freqs,dims)**self.d*q,axis=self.axis)
        else:
            return np.fft.ifft(np.reshape(1j*freqs,dims)**self.d*q,axis=self.axis).real.astype(x.dtype)
