...
class PDELibrary(BaseFeatureLibrary):
    ...
    def __init__(
        ...
        if derivative_order < 0 or derivative_order > 3:
            raise ValueError("The derivative order must be " "non-negative")
    ...
    def get_feature_names(self, input_features=None):
        ...
        for k in range(len((self.spatial_grid).shape) * self.derivative_order):
            feature_names.append(
                self.function_names[len(self.functions) + k](
                    *[input_features[j] for j in range(self.n_input_features_)]
                )
            )
        for k in range(len((self.spatial_grid).shape) * self.derivative_order):
            for i, f in enumerate(self.functions):
    ...
    def fit(self, x, y=None):
        ...
        if self.spatial_grid is not None:
            ...
            #     u_derivs = zeros((num_gridx, num_gridy, num_time,
            #                       self.derivative_order,
            #                       self.derivative_order))
            #     for i in range(num_time):
            #         for j in range(self.derivative_order):
            #             for jj in range(j, self.derivative_order):
            #                 for k in range(num_gridy):
            #                     u_derivs[:, k, i, j, jj] = FiniteDifference(
            #                         d=j)._differentiate(
            #                         u[:, k, i, j, jj], spatial_grid[:, k])
        ...
        if self.spatial_grid is not None:
            ...
            n_output_features += (
                n_output_features
                * len((self.spatial_grid).shape)
                * self.derivative_order
            )
            # Add the pure derivative library terms
            n_output_features += len((self.spatial_grid).shape) * self.derivative_order
        ...
        if self.spatial_grid is not None:
            ...
            self.function_names = hstack(
                (
                    ...
                    list(
                        map(
                            lambda i: (lambda *x: "".join(x) + "_" + "z" * i),
