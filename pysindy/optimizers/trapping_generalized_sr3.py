...
class TrappingGeneralizedSR3(SR3):
    """
    ...
    >>> from pysindy.optimizers import TrappingGeneralizedSR3
    >>> lorenz = lambda z,t : [10*(z[1] - z[0]),
    >>>                        z[0]*(28 - z[2]) - z[1],
    >>>                        z[0]*z[1] - 8/3*z[2]]
    >>> t = np.arange(0,2,.002)
    >>> x = odeint(lorenz, [-8,8,27], t)
    >>> opt = TrappingGeneralizedSR3(threshold=0.1)
    ...
    def __init__(
        ...
        super(TrappingGeneralizedSR3, self).__init__(
            threshold=threshold,
            max_iter=max_iter,
    ...
    def _solve_m_relax_and_split(self, r, N, m_prev, m, A, coef_sparse, tk_previous):
        ...
        # PQW = np.tensordot(self.mod_matrix, PQW, axes=([1, 1]))
        A_b = (A - PW) / self.eta
    ...
    def _solve_direct_cvxpy(self, r, N, x_expanded, y, Pmatrix, coef_prev):
        ...
        if np.all(self.PL_ == 0) and np.all(self.PQ_ == 0):
            ...
            cost_m = cp.lambda_max(
                #cp.reshape(Ls - self.mod_matrix @ (m_cp @ Q), (r, r))
                self.mod_matrix @ cp.reshape(Ls - m_cp @ Q, (r, r))
            )
            # cost_m = cp.lambda_max(
            #     cp.reshape(Ls - m_cp @ Q, (r, r))
            # )
    ...
    def _reduce(self, x, y):
        ...
        TrappingGeneralizedSR3 algorithm.
        ...
        if k == self.max_iter - 1:
            warnings.warn(
                "TrappingGeneralizedSR3._reduce did not converge after {} iters.".format(
                    self.max_iter
                ),
                ConvergenceWarning,
            )
