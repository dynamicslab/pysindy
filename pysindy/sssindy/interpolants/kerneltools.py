...
Float1D = Any
Float2D = Any
# Scalar Kernel is a PDS kernel on R
ScalarKernel = Callable[[float, float], float]
VectorizedScalarKernel = Callable[[Float1D, Float1D], Float2D]
...
def vectorize_kfunc(k: ScalarKernel) -> VectorizedScalarKernel:
...
def get_kernel_block_ops(
    ...
    def k_super(x, y):
        """
        Returns:
            Kernel matrix with the layout:
        (n_ops_left, output_dim, n_ops_right, output_dim),
        reshaped to (n_ops_left * output_dim, n_ops_right * output_dim)
        """
