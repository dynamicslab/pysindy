...
def test_bad_parameters():
    ...
    with pytest.raises(ValueError):
        library = PolynomialLibrary(include_interaction=False, interaction_only=False)
    with pytest.raises(ValueError):
    ...
    with pytest.raises(ValueError):
        library = FourierLibrary(include_sin=False, include_cos=False)
    with pytest.raises(ValueError):
        library_functions = [lambda x: x, lambda x: x ** 2, lambda x: 0 * x]
        function_names = [
            lambda s: str(s),
...
@pytest.mark.parametrize(
    "library", [(PolynomialLibrary()), (FourierLibrary()), (data_custom_library)]
)
...
@pytest.mark.parametrize(
    "library, shape",
    [(PolynomialLibrary(), 10), (FourierLibrary(), 6), (data_custom_library, 9)],
)
